...
class Calculator{
    ...
    init(){
        ...
        addGroup(group: Groups(groupName: "Physics", functions: [Functions(functionName: "Force", formula: "mass * accleration = Force", variables: ["Mass", "Acceleration","Force"], results: [:]),Functions(functionName: "Velocity", formula: "Distance/Time=Velocity", variables: ["Distance","Time","Velocity"], results: [:]),Functions(functionName: "Kinetic Energy", formula: "(mass * velocity)/2", variables: ["mass","velocity","Kinetic Energy"], results: [:])]))
